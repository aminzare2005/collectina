...
اپ `vlonefarsi` روی Vercel/Liara هاست شده و IP ثابت ندارد.  
...
3. کاربر برای ورود به درگاه → از **خود `vlonefarsi.ir`** redirect می‌شود
4. بعد از پرداخت، کاربر → `vlonefarsi.ir` (callback)
...
| callbackUrl | backend | `https://vlonefarsi.ir/...` |
| Referer (دامنه ارجاع‌دهنده) | مرورگر هنگام ورود به درگاه | همان دامنه ثبت‌شده در پنل زیبال → `vlonefarsi.ir` |

`fetchme.ir` فقط برای API است.  
Referer باید `vlonefarsi.ir` باشد، نه `fetchme.ir`.
...
```text
[کاربر] → [vlonefarsi.ir] ──API──→ [fetchme.ir/proxy] → [gateway.zibal.ir API]
                |
                └── redirect به درگاه (Referer = vlonefarsi.ir)
                ↑
...
- callback را می‌سازد:
  - `https://vlonefarsi.ir/api/payment/verify?orderId=...`
- درخواست را می‌فرستد به:
  - `https://fetchme.ir/vlonefarsi/zibal-request.php`
...
- callback را validate می‌کند (باید روی `vlonefarsi.ir` باشد)
...
- کاربر redirect می‌شود به:
  - `https://vlonefarsi.ir/api/payment/start?trackId={trackId}`
- این route یک redirect 302 می‌دهد به:
  - `https://gateway.zibal.ir/start/{trackId}`

Referer هنگام ورود به درگاه = `vlonefarsi.ir` ✅
...
```text
https://vlonefarsi.ir/api/payment/verify?success=1&trackId=...&orderId=...
```
...
- درخواست verify می‌فرستد به:
  - `https://fetchme.ir/vlonefarsi/zibal-verify.php`
...
|---|---|
| `server/payment-proxy/config.php` | `public_html/vlonefarsi/config.php` |
| `server/payment-proxy/helpers.php` | `public_html/vlonefarsi/helpers.php` |
| `server/payment-proxy/zibal-request.php` | `public_html/vlonefarsi/zibal-request.php` |
| `server/payment-proxy/zibal-verify.php` | `public_html/vlonefarsi/zibal-verify.php` |
| `server/payment-proxy/zibal-test-flow.php` | `public_html/vlonefarsi/zibal-test-flow.php` (اختیاری — فقط تست) |
...
https://fetchme.ir/vlonefarsi/zibal-request.php
https://fetchme.ir/vlonefarsi/zibal-verify.php
https://vlonefarsi.ir/api/payment/start?trackId=...
```
...
- `allowed_callback_host` را `vlonefarsi.ir` نگه دار
...
این env variableها را در پروژه `vlonefarsi` تنظیم کن:
...
- [ ] ۴ فایل PHP اصلی روی `public_html/vlonefarsi/` آپلود شد
...
```text
https://fetchme.ir/vlonefarsi/zibal-test-flow.php?mode=app
```

این یک پرداخت تازه می‌سازد و از مسیر `vlonefarsi.ir/api/payment/start` به درگاه می‌فرستد.

فقط JSON بخواهی:

```text
https://fetchme.ir/vlonefarsi/zibal-test-flow.php?mode=json
```
...
راه‌حل: callback باید `https://vlonefarsi.ir/...` باشد.
...
1. **دامنه ثبت‌شده در پنل زیبال** با `NEXT_PUBLIC_APP_URL` یکی نیست  
   (مثلاً پنل `vlonefarsi.ir` ولی اپ روی `www.vlonefarsi.ir` یا `xxx.vercel.app`)
2. کاربر از URL دیگری checkout می‌زند (preview deploy)
3. redirect مستقیم به `gateway.zibal.ir` بدون عبور از `vlonefarsi.ir/api/payment/start`

راه‌حل:

- در پنل زیبال، دامنه ثبت‌شده را با `NEXT_PUBLIC_APP_URL` sync کن
- مطمئن شو deploy production روی `https://vlonefarsi.ir` است
...
- callback فقط برای host `vlonefarsi.ir` پذیرفته می‌شود
...
- `app/api/payment/start/route.ts` — redirect به درگاه از دامنه vlonefarsi.ir
...
|---|---|---|
| request | vlonefarsi.ir | fetchme.ir → zibal |
| ورود به درگاه | مرورگر | vlonefarsi.ir → gateway.zibal.ir |
| پرداخت کاربر | مرورگر | gateway.zibal.ir |
| callback | مرورگر | vlonefarsi.ir |
| verify | vlonefarsi.ir | fetchme.ir → zibal |

با این setup، محدودیت IP ثابت زیبال رعایت می‌شود و callback هم روی دامنه اصلی اپ (`vlonefarsi.ir`) می‌ماند.
